...
function renderAcceptedShares() {
  ...
  pools.forEach((pool, poolIndex) => {
    ...
    const sessionWorker = pool.session_best_difficulty == null ? "" : `<small class="best-worker">${pool.session_best_worker ? escapeHtml(pool.session_best_worker) : "Device unknown"}</small>`;
    const allTimeWorker = pool.all_time_best_difficulty == null ? "" : `<small class="best-worker">${pool.all_time_best_worker ? escapeHtml(pool.all_time_best_worker) : "Device unknown"}</small>`;
    summaries.push(`<article style="--pool-color:${color}"><header><i></i><strong>${escapeHtml(pool.name)}</strong><span>${escapeHtml(pool.coin || "BTC")} · ${shares.length}/10</span></header><dl><div><dt>Average last 10</dt><dd>${difficulty(pool.average_share_difficulty)}</dd></div><div><dt>Session best</dt><dd>${difficulty(pool.session_best_difficulty)}${sessionWorker}</dd></div><div><dt>All-time best</dt><dd>${difficulty(pool.all_time_best_difficulty)}${allTimeWorker}</dd></div><div><dt>${escapeHtml(pool.coin || "BTC")} network</dt><dd>${difficulty(pool.network_difficulty)}</dd></div></dl><small>${escapeHtml(pool.share_feed_message || (pool.share_feed_available ? "Actual accepted submissions" : "Accepted-share feed unavailable"))}</small></article>`);
  ...
  }).join("") : `<div class="empty"><strong>No accepted submissions retained yet.</strong><span>PoCiSys Public Pool Port and PoCiSys BCHN&SP supply exact accepted-share difficulty. Unsupported pool APIs remain explicitly N/A.</span></div>`;
}
...
$("#settings-form").addEventListener("submit", async event => {
  ...
  const payload = {
    ...
    pool_disconnect_grace_seconds: Number(form.elements.pool_disconnect_grace_seconds.value),
